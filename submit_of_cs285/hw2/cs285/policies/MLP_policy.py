...
class MLPPolicyPG(MLPPolicy):
    ...
    def update(self, observations, actions, advantages, q_values=None):
        ...
        loss.backward()
        if self.discrete:
            nn.utils.clip_grad_norm_(self.logits_na.parameters(), 2.5)
        else:
            nn.utils.clip_grad_norm_(self.mean_net.parameters(), 5)
        ...
        if self.nn_baseline:
            ...
            q_values = normalize(ptu.from_numpy(q_values), np.mean(q_values), np.std(q_values))
            pred_q_values = self.baseline(observations).squeeze()
            self.baseline_optimizer.zero_grad()
            loss_2 = self.baseline_loss(pred_q_values, q_values)
            loss_2.backward()
            nn.utils.clip_grad_norm_(self.baseline.parameters(), 2.5)
            self.baseline_optimizer.step()

        train_log = {
            'Training Loss': ptu.to_numpy(loss),
            'Baseline Loss': ptu.to_numpy(loss_2),
        }
        return train_log
