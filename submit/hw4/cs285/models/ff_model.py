...
class FFModel(nn.Module, BaseModel):
    ...
    def forward(
        ...
        obs_normalized = normalize(obs_unnormalized, ptu.from_numpy(obs_mean), ptu.from_numpy(obs_std)) # TODO(Q1)
        acs_normalized = normalize(acs_unnormalized, ptu.from_numpy(acs_mean), ptu.from_numpy(acs_std))# TODO(Q1)
        ...
        next_obs_pred = obs_unnormalized + unnormalize(delta_pred_normalized, 
                                                        ...
                                                        ptu.from_numpy(delta_std)) # TODO(Q1)
    ...
    def get_prediction(self, obs, acs, data_statistics):
        ...
        prediction = ptu.to_numpy(self(obs, acs, 
                                    ...
                                    data_statistics['delta_std'])[0])# TODO(Q1) get numpy array of the predicted next-states (s_t+1)
    ...
    def update(self, observations, actions, next_observations, data_statistics):
        ...
        target = self(observations, actions, 
                                    ...
                                    data_statistics['acs_std'], 
                                    data_statistics['delta_mean'], 
                                    data_statistics['delta_std'])[1]# TODO(Q1) compute the normalized target for the model.
        # Hint: you should use `data_statistics['delta_mean']` and
        # `data_statistics['delta_std']`, which keep track of the mean
        # and standard deviation of the model.

        loss =  self.loss(target, delta_normalize)# TODO(Q1) compute the loss
        # Hint: `self(...)` returns a tuple, but you only need to use one of the
        # outputs.
